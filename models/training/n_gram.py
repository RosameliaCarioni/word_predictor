...
class NGram:
    ...
    def count_n_grams(self, line):
        ...
        for i,word in enumerate(words):
            # FOUR GRAM 
            previous_words = self.get_previous_words(words, i, self.N -1) # 3 because of four gram 
            ...
            # TRI GRAM
            previous_words = self.get_previous_words(words, i, self.N -2) # 3 because of four gram 
            ...
            # BI GRAM
            previous_words = self.get_previous_words(words, i, self.N -3) # 3 because of four gram 
            if len(previous_words) == self.N - 3:
                if word in self.bigram_count:
                    if previous_words in self.bigram_count[word]:
                        self.bigram_count[word][previous_words[0]] += 1
                    else:
                        self.bigram_count[word][previous_words[0]] = 1
                else:
    ...
    def probability_n_gram(self):
        for word in self.bigram_count.keys():
            ...
            for prev_word in prev_words.keys():
                ...
                self.bigram_prob[word][prev_word] = math.log(self.bigram_count[word][prev_word]/self.unigram_count[self.id_to_word[prev_word]]) # p (word|prev_word) =  c(word|prev_word)/ c(prev_word)

        for word in self.trigram_count.keys():
            prev_words = self.trigram_count[word]
            for prev_word in prev_words: 
                self.trigram_prob[word] = {}
                self.trigram_prob[word][prev_word] = math.log(self.trigram_count[word][prev_word]/self.bigram_count[self.id_to_word[prev_word[1]]][prev_word[0]])

        for word in self.fourgram_count.keys(): 
            prev_words = self.fourgram_count[word]
            for prev_word in prev_words: 
                self.fourgram_prob[word] = {}
                self.fourgram_prob[word][prev_word] = math.log(self.fourgram_count[word][prev_word]/self.trigram_count[self.id_to_word[prev_word[-1]]][prev_word[:-1]])
...
def main():
    ...
    file_path = '/Users/rosameliacarioni/University/MSc/1_year/4_period/language engineering/word_predictor/testin.txt'
